<template>
  <p>
    <i v-show="getCartNumber.cartNumber" class="sub-btn">
      <svg @click.stop="clickSub({isAdd:false,food})" :class="{icon:true,noSub:getCartNumber.kinds>1}"
           aria-hidden="true">
        <use xlink:href="#icon-jian2"></use>
      </svg>
    </i>
    <span v-show="getCartNumber.cartNumber">{{getCartNumber.cartNumber}}</span>
    <i class="add-btn">
